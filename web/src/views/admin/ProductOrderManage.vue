        ...
        <el-table ref="tableComponents"
          ...
          <el-table-column prop="status" label="状态" width="120">
            <template #default="scope">
              ...
              <el-tag v-else-if="scope.row.status==='待配送'" type="info">{{ scope.row.status }}</el-tag>
              <el-tag v-else-if="scope.row.status==='骑手配送中'" type="primary">{{ scope.row.status }}</el-tag>
          ...
          <el-table-column prop="riderInfo" label="骑手信息"></el-table-column>
          <el-table-column prop="remark" label="备注" show-overflow-tooltip></el-table-column>
          <el-table-column prop="createTime" label="创建时间"></el-table-column>
          <el-table-column fixed="right" label="高级操作" width="120">
            <template #default="scope">
              <el-button type="primary" v-if="scope.row.status==='待配送'" :icon="Position"
                         @click="delivery(scope.$index, scope.row)">配送
              </el-button>
            </template>
          </el-table-column>
    ...
    >
      <el-form ref="formRef" :model="formData" label-width="100px">
        <slot name="content">
          ...
          <el-form-item label="骑手信息" prop="riderInfo">
            <el-input v-model="formData.riderInfo"></el-input>
          </el-form-item>
...
const statusList = ref(['待支付', '待配送', '骑手配送中', '已完成', '已取消'])
...
function delivery(index, row) {
  ElMessageBox.prompt('请输入骑手信息（姓名+电话）', '提示', {
    confirmButtonText: '确认',
    cancelButtonText: '取消',
    inputPattern: /^.{1,}$/, // 这里设置你的正则表达式，例如至少输入一个字符
    inputErrorMessage: '骑手信息不能为空' // 当输入不满足正则表达式时显示的错误信息
  }).then(({value}) => {
    request.post("/productOrder/delivery/" + row.id + "?riderInfo=" + value).then(res => {
      if (!res) {
        return
      }
      ElMessage({
