...
public class ProductOrderServiceImpl implements ProductOrderService {
    ...
    public void cancel(Integer id) {
        ProductOrder productOrder = selectById(id);
        if (!productOrder.getStatus().equals("待配送") && !productOrder.getStatus().equals("待支付")) {
            throw new CustomException("数据已过期，请先刷新页面");
        }
        //返回库存
        productService.in(productOrder.getProductId(), productOrder.getQuantity());
        //退款 只有已付款的才退款
        if (productOrder.getStatus().equals("待配送")) {
     ...
     *
     * @param id
     * @param riderInfo 配送单号
     */
    public void delivery(Integer id, String riderInfo) {
        ProductOrder productOrder = selectById(id);
        if (!productOrder.getStatus().equals("待配送")) {
            throw new CustomException("数据已过期，请先刷新页面");
        }
        productOrder.setStatus("骑手配送中");
        productOrder.setRiderInfo(riderInfo);
        updateById(productOrder);
    }
    ...
    public void confirm(Integer id) {
        ProductOrder productOrder = selectById(id);
        if (!productOrder.getStatus().equals("骑手配送中")) {
            throw new CustomException("数据已过期，请先刷新页面");
        }
